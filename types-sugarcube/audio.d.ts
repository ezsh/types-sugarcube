...
export interface AudioTrack {
	...
	loop(): boolean;
	/**
	 * Sets the track's repeating playback state (default: false). When used to set the loop state, returns
	 * @param state The loop state.
	 * @returns a reference to the current AudioTrack instance for chaining.
	 * @since 2.28.0
	 */
	loop(state: boolean): AudioTrack;
	 ...
	 * @since 2.28.0
	 */
	time(seconds: number): AudioTrack;
	 ...
	 * @since 2.28.0
	 */
	volume(level: number): AudioTrack;
}
...
export interface AudioRunner {
	 ...
	 * @since 2.28.0
	 */
	off(events: string | object | JQuery.Event, selector?: string, handler?: (event: JQuery.Event) => void): AudioRunner;
	 ...
	 * @since 2.28.0
	 */
	on(events: string | object | JQuery.Event, selector?: string, data?: any, handler?: (event: JQuery.Event) => void): AudioRunner;
	 ...
	 * @since 2.28.0
	 */
	one(events: string | object | JQuery.Event, selector?: string, data?: any, handler?: (event: JQuery.Event) => void): AudioRunner;
...
export interface AudioList {
	 ...
	 * @since 2.28.0
	 */
	shuffle(state: boolean): AudioList;
...
export interface SimpleAudio {
	...
	/**
	 * Displays the loading screen until all currently registered audio tracks have either loaded to a playable state or aborted loading due to errors. The loading process is as described in @see SimpleAudio.load().
	 * WARNING: This should not be done lightly if your audio sources are on the network, as it forces players to begin downloading them.
	 * @since 2.28.0
	 */
	loadWithScreen(): void;
	 ...
	 */
	select(selector: string): AudioRunner;
	...
	tracks: {
		 ...
		 * @since 2.28.0
		 */
		add(trackId: string, ...sources: string[]): void;
		/**
	...
	groups: {
		 ...
		 */
		add(groupId: string, ...trackIds: string[]): void;
	...
	lists: {
		 ...
		 * );
		 */
		add(listId: string, ...sources: string[] | object[]): void;
